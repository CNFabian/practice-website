    ...
    factors.forEach(factor => {
      switch (factor.id) {
        ...
        case 'mix':
          ...
          break;
        case 'inquiries':
          if ((factor.value as number) > 2) {
            improvements.push({
              factor: 'New Credit',
              impact: 'Low',
              suggestion: 'Limit new credit applications for the next 6-12 months'
            });
          }
          break;
      }
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {[
            { range: '800-850', level: 'Exceptional', color: 'bg-green-100 text-green-800', description: 'Best rates and terms' },
            { range: '740-799', level: 'Very Good', color: 'bg-blue-100 text-blue-800', description: 'Good rates available' },
            { range: '670-739', level: 'Good', color: 'bg-indigo-100 text-indigo-800', description: 'Fair rates offered' },
            { range: '580-669', level: 'Fair', color: 'bg-yellow-100 text-yellow-800', description: 'Higher rates likely' },
            { range: '300-579', level: 'Poor', color: 'bg-red-100 text-red-800', description: 'Difficulty getting credit' }
          ].map((category) => (
            <div key={category.range} className={`p-4 rounded-xl ${category.color}`}>
              <RobotoFont weight={600} className="text-sm mb-1">
                {category.level}
              </RobotoFont>
              <RobotoFont className="text-xs mb-2">
                {category.range}
              </RobotoFont>
        ...
        {currentCategory.level !== 'Exceptional' && (
          <div className="mt-4 p-4 bg-blue-50 rounded-xl">
            <RobotoFont className="text-sm text-blue-800">
              <strong>Keep up the great work!</strong> Continue monitoring and improving your credit habits to reach the next level.
            </RobotoFont>
          </div>
        )}
      </div>
